...
class TeacherReturnedFromLeaveProcessor(TeacherActionProcessor):
    ...
    def _set_statuses(self) -> None:
        if self.teacher.has_groups:
            ...
            self.teacher.project_status = TeacherProjectStatus.NO_GROUP_YET
        self.teacher.status_since = self.timestamp
        self.teacher.save()

    def _create_log_events(self) -> None:
        TeacherLogEvent.objects.create(teacher=self.teacher, type=TeacherLogEventType.RETURNED_FROM_LEAVE)
