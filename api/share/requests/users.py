...
from api.share.constants import (
    LENGTH_REGISTRATION_BOT_CHAT_ID,
    MAX_LENGTH_EMAIL,
    MAX_LENGTH_FIRST_LAST_NAME,
    MAX_LENGTH_PHONE,
    MAX_LENGTH_TG_USERNAME,
    MIN_LENGTH_TG_USERNAME,
)
...
class GetUsersCountRequest(ValidRequestObject):
    ...
    def from_dict(cls, input_dict):
        ...
        if (
            input_dict.get("registration_bot_chat_id")
            and len(input_dict.get("registration_bot_chat_id")) > LENGTH_REGISTRATION_BOT_CHAT_ID
        ):
            invalid_request.add_error("registration_bot_chat_id", "Invalid length")

        if input_dict.get("tg_username") and (
            len(input_dict.get("tg_username")) < MIN_LENGTH_TG_USERNAME
            or len(input_dict.get("tg_username")) > MAX_LENGTH_TG_USERNAME
        ):
            invalid_request.add_error("tg_username", "Invalid length")

        # See modal PersonalInfo
        if (
            input_dict.get("first_name")
            and len(input_dict.get("first_name")) > MAX_LENGTH_FIRST_LAST_NAME
        ):
            invalid_request.add_error("first_name", "Invalid length")

        # See modal PersonalInfo
        if (
            input_dict.get("last_name")
            and len(input_dict.get("last_name")) > MAX_LENGTH_FIRST_LAST_NAME
        ):
            invalid_request.add_error("last_name", "Invalid length")

        if input_dict.get("email") and not re.match(
            r"^[^@\s]+@[^@\s]+\.[^@\s]+$", input_dict.get("email")
        ):
            invalid_request.add_error("email", "Incorrect format")

        if input_dict.get("email") and len(input_dict.get("email")) > MAX_LENGTH_EMAIL:
        ...
        if input_dict.get("phone") and len(input_dict.get("phone")) > MAX_LENGTH_PHONE:
        ...
        return cls(
            registration_bot_chat_id=input_dict.get("registration_bot_chat_id"),
