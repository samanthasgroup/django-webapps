...
def test_languages_and_levels_list(admin_client, language):
    ...
    response = admin_client.get(f"/api/languages_and_levels/{query_string}")
...
def test_languages_and_levels_retrieve(admin_client):
    language_and_level = LanguageAndLevel.objects.first()
    response = admin_client.get(f"/api/languages_and_levels/{language_and_level.id}/")
